...
# USAGE: ./katanamainnet_fork.sh
set -e

CWD="$( cd -- "$(dirname "$0")" >/dev/null 2>&1 ; pwd -P )"

CHAIN_ID="${CHAIN_ID:-katanamain-local-testnet}"
...
KATANAD_BIN_CURRENT="${KATANAD_BIN_CURRENT:-$FORK_DIR/../../build/katanad}"

# Loads another sources ,
# It will download the mainnet binaries
. $CWD/download-mainnet-katanad.sh

# It will download the mainnet genesis
KATANAMAINNET_GENESIS_PATH=$KATANAMAINNET_GENESIS_PATH . $CWD/tinker-mainnet-genesis.sh
...
$KATANAD_BIN_MAINNET $nodeHome $cid init $ADMIN_KEY

echo Create node admin wallet
yes "$NODE_MNEMONIC$NEWLINE" | $KATANAD_BIN_MAINNET $nodeHome keys add $ADMIN_KEY $kbt --recover
NODE_ADDR="$($KATANAD_BIN_MAINNET $nodeHome keys show $ADMIN_KEY -a $kbt)"
...
cp $KATANAMAINNET_GENESIS_PATH $nodeDir/$genesisConfigPath
...
perl -i -pe 's|minimum-gas-prices = ""|minimum-gas-prices = "0.05ukatana"|g' $nodeApp

nodeLogPath=$hdir.katanad-main.log
unset KATANA_ENABLE_BETA

pid_path=$nodeDir.pid
KATANA_ENABLE_BETA=false $KATANAD_BIN_MAINNET $nodeHome start --grpc.address="0.0.0.0:9090" --x-crisis-skip-assert-invariants --grpc-web.enable=false --log_level $LOG_LEVEL > $nodeLogPath 2>&1 &
...
CHAIN_ID=$CHAIN_ID KATANAD_BIN=$KATANAD_BIN_CURRENT wait_until_block $WAIT_UNTIL_HEIGHT
echo "Finish wait_until_block"

CURRENT_BLOCK_HEIGHT=$(CHAIN_ID=$CHAIN_ID KATANAD_BIN=$KATANAD_BIN_CURRENT get_block_current_height)
...
KATANAD_V1_PID_FILE=$pid_path CHAIN_DIR=$CHAIN_DIR CHAIN_ID=$CHAIN_ID LOG_LEVEL=$LOG_LEVEL NODE_NAME=node UPGRADE_TITLE=$UPGRADE_TITLE KATANAD_BIN_V1=$KATANAD_BIN_MAINNET KATANAD_BIN_V2=$KATANAD_BIN_CURRENT $CWD/upgrade-test-single-node.sh
...
echo
echo "👍 Upgrade Process Finish to $KATANAD_BIN_CURRENT"
echo
